...
class Auth {
  ...
  Future<String?> authenticate(
      {String? id, String? domain, String? password}) async {
    String time = DateTime.now().millisecondsSinceEpoch.toString();
    String tranNo = time.substring(time.length - 4);
    ...
    try {
      Response rsp = await post(Uri.parse(url! + _AUTH),
      ...
      if (response.common!.resCode != '000') {
        print(
            'Authentication failed: [${response.common!.resCode}]${response.common!.resMessage}');
        return null;
      }
...
class AuthRequest {
  ...
  AuthRequest({this.common, this.content});
...
class AuthResponse {
  ...
  AuthResponse({this.common, this.content});
...
class AuthRequestContent {
  ...
  AuthRequestContent({this.id, this.domain, this.password, this.token});
...
class _AuthResponseContent {
  ...
  _AuthResponseContent.fromJson(Map<String, dynamic> json)
      : token = json['token'];
